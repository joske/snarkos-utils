...
for n in $NODES; do
	echo "restart $n"
	ssh -F ~/.ssh/devnet/config -i ~/.ssh/devnet/devnet-key -o StrictHostKeyChecking=no -o ConnectTimeout=5 "$n" "sudo systemctl stop snarkos && sudo systemctl start snarkos"
	if [ $? -ne 0 ]; then
