...
if ARGV.length != 1
  puts 'Usage: start.rb <number of nodes>'
  exit 1
end
...
def wait_credits(num, address)
  puts "waiting for node #{num} to be credited #{address}"
  uri = URI("http://0.0.0.0:3030/mainnet/program/credits.aleo/mapping/account/#{address}")
  loop do
    begin
      response = Net::HTTP.get(uri)
      credits = response.tr!('"', '').sub!('u64', '').to_i
      break if credits >= 1_000_000_000_000
    rescue StandardError
      nil
    end
    sleep 5
  end
  puts "node #{num} is credited #{address}"
end

def transfer(num, priv, address)
  transfer = "cargo run -- developer execute credits.aleo transfer_public #{address} \"10000000000000u64\" " \
         "--private-key #{priv} --query http://0.0.0.0:3030 " \
         '--broadcast http://0.0.0.0:3030/mainnet/transaction/broadcast'
  puts "transferring credits to #{num} #{address}"
  system(transfer, out: File::NULL, err: File::NULL) || puts("\u001b[31mtransferring credits to #{num} failed\u001b[0m")
  wait_credits(num, address)
end

def wait_up
  puts 'waiting for node 0 to be up'
  response = nil
  while response.nil?
    sleep 10
    begin
      response = Net::HTTP.get_response(URI('http://0.0.0.0:3030/mainnet/latest/height'))
    rescue StandardError
      nil
    end
  end
...
def wait_bond(num, address)
  puts "waiting for node #{num} to be bonded #{address}"
  ...
  puts "node #{num} is bonded #{address}"
end

def bond(num, priv, address)
  bond = "cargo run -- developer execute credits.aleo bond_public #{address} \"1000000000000u64\" " \
         "--private-key #{priv} --query http://0.0.0.0:3030 " \
         '--broadcast http://0.0.0.0:3030/mainnet/transaction/broadcast'
  puts "bonding node #{num} #{address}"
  system(bond, out: File::NULL, err: File::NULL) || puts("\u001b[31mbonding node #{num} failed\u001b[0m")
  wait_bond(num, address)
end
...
0..NUM.times do |i|
  ...
  if i > 3
    ...
    priv = n[i]['private_key']
    address = n[i]['address']
    transfer(i, priv0, address)
    bond(i, priv, address)
  end

  puts "starting node #{i}: #{cmd}"
  pid = spawn cmd
  pids << pid
end
